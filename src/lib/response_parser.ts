...
class ResponseParser {
  ...
  public async extractMessage(response: string): Promise<string | null> {
    try {
      const messageMatch = /<message>(.*?)<\/message>/s.exec(response);
      if (messageMatch) {
  ...
  public async parseFunctionUsage(response: string): Promise<{
    ...
    try {
      ...
      console.error("Error parsing function in response:", error);
      return { functionName: null, functionArgs: null };
    }
  }
}
